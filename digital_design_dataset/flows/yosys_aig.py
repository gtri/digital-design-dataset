...
def yosys_aig(
    verilog_files: list[Path], yosys_bin: str = "yosys"
...
def raw_aig_to_tree(raw_aig: str) -> nx.DiGraph:
    ...
    # 3. Find header-indicated number of latches and add them to list of nodes.
    for i in range(
        (1 + input_count), (1 + input_count + latch_count)
    ...
    for i in range(
        (1 + input_count + latch_count), (1 + input_count + latch_count + output_count)
    ...
    for i in range(
        ...
        if aig_graph.has_node(f"{node_index}_out"):
            ...
            aig_graph.add_edge(
                node_index, f"{node_index}_out", index=node_index, type="STANDARD"
            )
        elif aig_graph.has_node(f"{node_index + 1}_out"):
            # Found inversed output. Add edge.
            aig_graph.add_edge(
                node_index, f"{node_index + 1}_out", index=node_index, type="INVERSE"
            )
