...
class DatasetRetriever:
    dataset_name: str
    dataset_type: str  # TODO: Chnage to be more like tags, like a list[str]
...
class OpencoresDatasetRetriever(DatasetRetriever):
    ...
    # Manually curated and included in this library

    DATASET_FP = CURRENT_DIR / "data" / "opencores.zip"

    def get_dataset(self, overwrite: bool = False) -> None:
        z = zipfile.ZipFile(str(self.DATASET_FP))
        designs = []
...
class OPDBDatasetRetriever(DatasetRetriever):
    ...
    def get_dataset(self, overwrite: bool = False) -> None:
        ...
        for design in design_list.splitlines():
            ...
            with open(source_file_dir / (design_name + ".v"), "w") as f:
                f.write(text)
